...
backends = ["'Sequential'", "'MPI'",
            "'OpenCL GPU Scheme A'", "'OpenCL GPU Scheme B'", "'OpenCL GPU Scheme C'", "'OpenCL GPU Scheme D'"]
    ...
    for disc_2 in discretisations:
        ...
        for layer in [1, 2, 4, 8, 16, 32, 64, 96, 128, 160, 192, 224, 256]:
            ...
            try:
                ...
                parloop_min = profile_data.min().sum(axis=1)
        ...
        with open(os.path.join(graph_dir, "{}-{}-{}-runtime-increasing-layers-table.tex".format(test_name, discretisation, mesh_size)), 'w') as f:
            latex = df.to_latex()
            f.write(latex)
